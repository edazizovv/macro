...
with warnings.catch_warnings():
    ...
    for k in range(100):
        ...
        for t in range(100):
            ...
            res = pandas.DataFrame(data={'perf_r2_w': [r2_tt2st_w],
...
respo_mean = respo.groupby(by=['k'])[['perf_r2_w', 'perf_sd_w']].mean()
respo_median = respo.groupby(by=['k'])[['perf_r2_w', 'perf_sd_w']].median()
respo_min = respo.groupby(by=['k'])[['perf_r2_w', 'perf_sd_w']].min()
...
x_base = x_bases[
    respo_median[respo_median['perf_sd_w'] == respo_median['perf_sd_w'].max()].index[0]
].tolist()
...
for t in range(100):
    ...
    sds = []
    for xb in x_factors_selection:
        sd = somersd(y_train.values, x_train[xb].values)
        sds.append(sd)
    sdr = pandas.DataFrame(data={'drivers': x_factors_selection,
                                 'sd': sds,
                                 't': t})
    sd_res.append(sdr)
sd_res = pandas.concat(sd_res, axis=0, ignore_index=True)
sd_res_avg = sd_res.groupby(by='drivers')['sd'].median()
x_factors_selection_new = sd_res_avg[sd_res_avg.abs() >= sd_thresh].index.values
...
while not stop:
    ...
    for k in range(100):
        ...
        x_left = [x for x in x_factors_selection_new if x not in x_base]
        ...
        for j in range(len(x_base)):
            ...
            x_train = result.loc[x_ix_train, [x for x in x_base if x != x_base[j]]].values
            y_train = result.loc[y_ix_train, target].values
            x_test = result.loc[x_ix_test, [x for x in x_base if x != x_base[j]]].values
            y_test = result.loc[y_ix_test, target].values
            ...
            res = pandas.DataFrame(data={'drivers': [x_base[j]],
                                         'perf_r2_w': [r2_tt2st_w],
                                         'perf_sd_w': [sd_tt2st_w],
                                         'perf_r2_wo': [r2_tt2st_wo],
                                         'perf_sd_wo': [r2_tt2st_wo],
    ...
    best_candidate_add_perf = respox_median.loc[best_candidate, 'perf_sd_wo']

    respoz = pandas.concat(respoz, axis=0, ignore_index=True)
    respoz['diff_r2'] = respoz['perf_r2_w'] - respoz['perf_r2_wo']
    respoz['diff_sd'] = respoz['perf_sd_w'] - respoz['perf_sd_wo']
    respoz_mean = respoz.groupby(by=['drivers'])[['perf_r2_w', 'diff_r2', 'perf_sd_w', 'diff_sd', 'perf_sd_wo']].mean()
    respoz_median = respoz.groupby(by=['drivers'])[['perf_r2_w', 'diff_r2', 'perf_sd_w', 'diff_sd', 'perf_sd_wo']].median()
    respoz_min = respoz.groupby(by=['drivers'])[['perf_r2_w', 'diff_r2', 'perf_sd_w', 'diff_sd', 'perf_sd_wo']].min()

    best_candidate = respoz_median[respoz_median['perf_sd_wo'] == respoz_median['perf_sd_wo'].max()].index[0]
    best_candidate_ex_perf = respoz_median.loc[best_candidate, 'perf_sd_wo']

    if (best_candidate_add_perf > previous_perf) and (best_candidate_add_perf > best_candidate_ex_perf):
        stop = False
        x_base += [best_candidate]
        previous_perf = best_candidate_add_perf
    elif (best_candidate_ex_perf > previous_perf) and (best_candidate_ex_perf > best_candidate_add_perf):
        stop = False
        x_base = [x for x in x_base if x != best_candidate]
        previous_perf = best_candidate_ex_perf
    else:
        stop = True

    if (len(x_base) == len(x_factors_selection_new)) or (len(x_base) < 1):
        stop = True
