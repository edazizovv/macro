...
sd_thresh_primodial = 0.2
x_factors_selection_primodial = sd_res_avg[sd_res_avg.abs() >= sd_thresh_primodial].index.values
...
with warnings.catch_warnings():
    ...
    for k in range(500):
        ...
        respo.append(sd_tt2st_w)

x_bases_bazillion = numpy.unique([y for x in x_bases for y in x])
...
listeds = []
for j in range(30):
    for c in x_bases_bazillion:

        r = numpy.random.choice(list(range(len(x_bases))))
        x_base = x_bases[r]
        ...
        if c in x_base:
            ...
            x_train = result.loc[x_ix_train, [z for z in x_base if z != c]].values
            y_train = result.loc[y_ix_train, target].values
            x_test = result.loc[x_ix_test, [z for z in x_base if z != c]].values
            y_test = result.loc[y_ix_test, target].values
        else:
            sd_wo = respo[r]

            x_train = result.loc[x_ix_train, x_base.tolist() + [c]].values
            y_train = result.loc[y_ix_train, target].values
            x_test = result.loc[x_ix_test, x_base.tolist() + [c]].values
            y_test = result.loc[y_ix_test, target].values
        ...
        perf_w_st.append(sd_w)
        perf_wo_st.append(sd_wo)
        listeds.append(c)

from scipy import stats
def lower_bound(x):
    mv = x.mean()
    sv = numpy.std(x.values, ddof=1)
    av = 0.05
    nv = 30
    mv_ci = mv + stats.t.ppf(q=(av / 2), df=(nv - 1)) * sv / (nv ** 0.5)
    return mv_ci

reestimated = pandas.DataFrame(data={'driver': listeds,
                                     'sd_w': perf_w_st,
                                     'sd_wo': perf_wo_st})
reestimated['sd_diff'] = reestimated['sd_w'] - reestimated['sd_wo']
reestimated_agg = reestimated.groupby(by='driver')['sd_diff'].agg(lower_bound)
reestimated_agg_means = reestimated.groupby(by='driver')['sd_diff'].mean()
reestimated_agg = reestimated_agg.sort_values()
reestimated_agg_means = reestimated_agg_means.sort_values()
x_base = reestimated_agg[reestimated_agg >= 0].index.values.tolist()
x_base.reverse()
...
while not stop:
    ...
    for k in range(100):
        ...
        x_left = [x for x in x_factors_selection if x not in current_base]
        # x_left = [x for x in x_factors_selection_primodial if x not in current_base]
        ...
        for j in range(len(x_left)):
            ...
            x_train = result.loc[x_ix_train, current_base + [x_left[j]]].values
            y_train = result.loc[y_ix_train, target].values
            x_test = result.loc[x_ix_test, current_base + [x_left[j]]].values
            y_test = result.loc[y_ix_test, target].values
    ...
    if best_candidate_perf > previous_perf:
        stop = False
        current_base += [best_candidate]
        previous_perf = best_candidate_perf
    else:
        stop = True

    if len(current_base) == len(x_factors_selection):     # x_factors_selection_primodial
        stop = True

skeleton_base = x_base[:taken_ix+1]
complement_base = [x for x in current_base if x not in skeleton_base]
