...
class NN(nn.Module):
    ...
    def fit(self, X_train, y_train, X_val, y_val, loss_function):
        ...
        self.initialise()
        self.optimiser = self._optimiser(self.parameters(), **self._optimiser_kwargs)
        self.scheduler = self._scheduler(self.optimiser, **self._scheduler_kwargs)
        ...
        for i in range(self._epochs):
            ...
            for phase in ['train', 'validate']:
                ...
                if phase == 'train':
                    train_lost = single_loss.item()
                    self.aggregated_losses.append(train_lost)
                    single_loss.backward()
                    self.optimiser.step()
                else:
                    validation_lost = single_loss.item()
            ...
            # self.scheduler.step(metrics=train_lost)
            self.scheduler.step()
    ...
    def should_have_stopped(self, q=0.1):
        thresh = numpy.quantile(self.aggregated_losses, q=q)
        terminal_epoch = numpy.where(numpy.array(self.aggregated_losses) <= thresh)[0][-1]
