...
# fit_base <- auto.arima(y_sub_t, trace=FALSE)
...
for (j in 1:fo) {
  ...
  y_sub <- y_dev[(ixx * (j - 1))+1:(ixx * j)+1-1]
  y_sub_t <- ts(y_sub, frequency=12)
  fit_base <- auto.arima(y_sub_t, trace=FALSE)
  # orders_base <- c(0, 2, 0, 2, 12, 1, 1)
  orders_base <- c(0, 1, 0, 1, 12, 1, 0)
  for (i in 1:fold_n) {
    z <- z + 1
    ...
    fit <- Arima(y_sub_t, 
                 order=c(orders_base[1], orders_base[6], orders_base[2]), 
                 seasonal=c(orders_base[3], orders_base[7], orders_base[4]),
                 method="ML")  
    fc <- forecast(fit,h=1)
